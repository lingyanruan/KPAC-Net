...
def Defocus_Deblur_Net6_ms(t_image, ks = 5, bs=2, ch=48, is_train=False, reuse=False, hrg=128, wrg=128, name="deblur_net"):
    ...
    for i in range(bs):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            n_sh = MeanPool2d(n, filter_size=(int(hrg/4), int(wrg/4)), strides=(int(hrg/4), int(wrg/4)), padding='SAME', name='sha_pool/%s' % (i))
            ...
            n_sh = UpSampling2dLayer(n_sh, [int(hrg/4),int(wrg/4)], is_scale=False, method=1, align_corners=False, name='sha_attention/%s' % (i))
...
def Defocus_Deblur_Net6_ds(t_image, ks = 5, bs = 2, is_train=False, reuse=False, hrg=128, wrg=128, name="deblur_net"):
    ...
    for i in range(bs):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            ## shape attention (global, shared)
            n_sh = MeanPool2d(n, filter_size=(int(hrg/8), int(wrg/8)), strides=(int(hrg/4), int(wrg/4)), padding='SAME', name='sha_pool/%s' % (i))
            ...
            n_sh = TileLayer(n_sh, [1,1,1,5], name='sha_tile/%s' % (i))
            n_sh = UpSampling2dLayer(n_sh, [int(hrg/8),int(wrg/8)], is_scale=False, method=1, align_corners=False, name='sha_attention/%s' % (i))
            nn = ElementwiseLayer([nn, n_sh], tf.multiply, 'sha_attention_mul/%s' % (i))
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        n = DeConv2d(n, 96, (4, 4), (int(hrg/4), int(wrg/4)), (2, 2), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, b_init=b_init, name='deblur/d0')
        ...
        n = DeConv2d(n, 48, (4, 4), (int(hrg/2), int(wrg/2)), (2, 2), act=tf.nn.leaky_relu, padding='SAME', W_init=w_init, b_init=b_init, name='deblur/d1')
        n = ConcatLayer([n, f2], 3, name='pre/s1')
